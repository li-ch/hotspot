...
int vector_setup(Vector* vector, size_t capacity, size_t element_size) {
	...
	vector->data = vmalloc(vector->capacity * element_size);

	return vector->data == NULL ? VECTOR_ERROR : VECTOR_SUCCESS;
}

int vector_copy(Vector* destination, Vector* source) {
	vector_is_initialized(source);
	vector_is_initialized(destination);
	...
	destination->data = vmalloc(destination->capacity * source->element_size);
...
void* iterator_next(Iterator* iterator) {
	void* current_itr = iterator->pointer;
...
int _vector_reallocate(Vector* vector, size_t new_capacity) {
	...
	if ((vector->data = vmalloc(new_capacity_in_bytes)) == NULL) {
		return VECTOR_ERROR;
	}
